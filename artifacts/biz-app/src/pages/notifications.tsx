...
import { Bell, AlertCircle, FileText, Sparkles, CheckCheck, CheckCircle2, RefreshCw } from "lucide-react";
...
const TYPE_CONFIG: Record<string, { icon: typeof AlertCircle; color: string; bg: string; label: string }> = {
...
type FilterTab = "all" | "unread" | "overdue_task" | "service_request" | "invoice_created" | "invoice_updated";
...
export default function NotificationsPage() {
  ...
  const TABS: { key: FilterTab; label: string; count?: number }[] = [
    ...
    { key: "overdue_task", label: "Overdue Tasks", count: typeCounts["overdue_task"] },
    { key: "service_request", label: "Client Requests", count: typeCounts["service_request"] },
    { key: "invoice_created", label: "Invoices", count: (typeCounts["invoice_created"] ?? 0) + (typeCounts["invoice_updated"] ?? 0) },
  ];
        ...
        <div className="flex items-center gap-2">
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-white bg-primary hover:bg-primary/90 rounded-xl px-3 py-2 shadow-sm transition-colors disabled:opacity-60"
            >
          ...
          >
            ...
            {tab.count != null && tab.count > 0 && (
              <span className={cn(
                ...
                filter === tab.key
                  ? tab.key === "unread" ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-600"
              ...
              return (
                <li
                  ...
                  className={cn(
                    ...
                    n.is_read ? "hover:bg-slate-50" : "bg-blue-50/30 hover:bg-blue-50/60"
                  )}
                >
                  {/* Icon */}
