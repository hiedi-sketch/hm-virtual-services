...
import { Play, Pause, Square, Timer, X, CheckCircle2, ChevronDown, StickyNote, UserCircle2 } from "lucide-react";
...
export function GlobalTimerBar() {
  ...
  // Inline client/task assign dropdown in the bar
  ...
  useEffect(() => {
    ...
    setAssignClientId(state.clientId ?? "");
    setAssignTaskId(state.taskId ?? "");
  }, [showAssign, state.clientId, state.taskId]);
  ...
  const assignClientTasks =
    tasks?.filter(t => assignClientId ? t.client_id === Number(assignClientId) : false) ?? [];

  function handleAssignApply() {
    if (assignClientId) {
      const client = clients?.find(c => c.id === Number(assignClientId));
      if (client) assignClient(client.id, client.contact_name?.trim() || client.name);
    }
    if (assignTaskId) {
      const task = tasks?.find(t => t.id === Number(assignTaskId));
      if (task) {
        assignTask(task.id, task.title);
  ...
  return (
    <div className={cn(
      ...
      <div className="max-w-7xl mx-auto h-12 flex items-center gap-4">
        ...
        <div className="relative flex items-center gap-2 min-w-0 flex-1" ref={assignRef}>
          <button
            type="button"
            onClick={() => setShowAssign(v => !v)}
            className="flex items-center gap-1.5 text-sm font-medium text-white/90 hover:text-white hover:bg-white/10 rounded-lg px-2 py-1 transition-colors min-w-0"
          >
            <UserCircle2 className="w-4 h-4 shrink-0 text-white/70" />
            <span className="truncate max-w-[200px]">
              {state.clientName || state.taskTitle
                ? [state.clientName, state.taskTitle].filter(Boolean).join(" · ")
                : <span className="text-white/50 italic font-normal">Assign client…</span>}
            </span>
            <ChevronDown className="w-3 h-3 shrink-0 text-white/60" />
          </button>
          {state.serviceType && <ServiceTypePill value={state.serviceType} />}
          ...
          {showAssign && (
            <div className="absolute left-0 top-full mt-2 w-72 bg-white rounded-xl shadow-xl border border-slate-200 z-50 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
                <p className="text-sm font-semibold text-slate-800">Assign to Timer</p>
              </div>
              <div className="p-4 space-y-3">
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Client</label>
                  <select
                    value={assignClientId}
                    onChange={e => { setAssignClientId(e.target.value ? Number(e.target.value) : ""); setAssignTaskId(""); }}
                ...
                {assignClientId && assignClientTasks.length > 0 && (
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Task <span className="font-normal normal-case text-slate-400">(optional)</span></label>
                    <select
                      value={assignTaskId}
                    ...
                    >
                      ...
                      {assignClientTasks.map(t => <option key={t.id} value={t.id}>{t.title}</option>)}
                    </select>
                  </div>
                )}
              </div>
              <div className="px-4 pb-4 flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 rounded-lg text-sm font-semibold bg-[#266b75] text-white hover:bg-[#1e5560] transition-colors"
                >
                  Apply
                </button>
                <button
