...
- **Invoices** (`/invoices`) — Full invoice lifecycle: create with line items (service picker + custom), edit, mark as paid (with date/method/notes), void, delete, send via email; status tabs (All/Draft/Sent/Unpaid/Paid/Void/Recurring); draft toggle, PDF download, Square pay-by-link; bell icon per invoice opens Reminder Modal; 4-card summary (Paid, Outstanding, Total, Overdue). Square sync: when an invoice is sent it is automatically created in the Square Dashboard (via Square Invoices API + Orders API) as UNPAID; voiding cancels it in Square. `square_invoice_id` stored on each record.
...
- **Payment Reminders** — Automated reminders at: due date, 3 days overdue, 5 days overdue, 10 days overdue; deduped in `invoice_reminders` DB table; manual send via bell icon → Reminder Modal; Square pay-link embedded in reminder emails
- **Team** (`/team`) — User management (create/edit/delete users, assign roles & client links); admin only
- **Client Portal** — Dedicated portal for client-role users: open tasks, hours used this month (reset-date-aware for VA), unpaid balance, invoice list; shows VA hours reset countdown. Estimate actions: "Accept & Get Started" opens StartServicesModal (choose immediate/future start + pay now or request invoice); "Request Changes" opens DeclineFeedbackModal (textarea for feedback). URL params ?onboard=ID and ?decline=ID auto-open the respective modal (used by estimate email buttons) and switch to the invoices tab.
...
- **Invoices**: id, client_id, lead_id (nullable), amount, type (invoice/estimate), status (draft/sent/paid/unpaid/void/accepted/declined), billing_type ("one_time"|"recurring", default "one_time"), due_date, description, line_items (json — each item has name, description, qty, unit_price, billing_type ("one_time"|"recurring")), notes, thank_you_message, paid_at, payment_method, payment_notes, recurring_id (nullable), service_period_start (nullable text), service_period_end (nullable text), decline_reason (nullable text — stores client feedback when estimate is declined), updated_at
...
- `POST /invoices/:id/send` — Send invoice/estimate via email. For estimates: includes "Accept & Get Started" + "Request Changes" buttons linking to portal with ?onboard=ID / ?decline=ID. For invoices: Square pay button.
- `POST /invoices/:id/start-services` — Client accepts estimate: marks estimate as "accepted", creates recurring series (auto_send=true), creates first invoice, optionally creates Square payment link. Body: {start_type: "immediate"|"future", start_date?: string, payment: "pay_now"|"request_invoice"}. Returns {success, first_invoice_id, recurring_id, payment_url}.
- `POST /invoices/:id/decline-with-feedback` — Client declines estimate with reason: marks as "declined", saves reason to decline_reason, notifies admins. Body: {reason: string}.
