...
- `POST /invoices/:id/send` — Send invoice/estimate via email. For estimates: includes "Accept & Get Started" + "Request Changes" buttons linking to portal with ?onboard=ID / ?decline=ID. For invoices: Stripe pay button.
- `POST /invoices/:id/start-services` — Client accepts estimate: marks estimate as "accepted", creates recurring series (auto_send=true), creates first invoice, optionally creates Stripe checkout session. Body: {start_type: "immediate"|"future", start_date?: string, payment: "pay_now"|"request_invoice"}. Returns {success, first_invoice_id, recurring_id, stripe_url}.
