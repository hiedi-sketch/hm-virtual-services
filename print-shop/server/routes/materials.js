...
const { materialSummary } = require('../utils/planning');
...
router.post('/', (req, res) => {
  ...
  const finalSku = req.body.sku || nextSku('materials', 'material');
  ...
  try {
    const id = db.prepare(
      `INSERT INTO materials (${keys.join(', ')}) VALUES (${keys.map(() => '?').join(', ')})`
...
router.put('/:id', (req, res) => {
  ...
  applyUpdate('materials', req.params.id, EDITABLE, req.body);
  const [material] = materialSummary(Number(req.params.id));
  res.json({ data: material });
});
...
router.post('/:id/adjust', (req, res) => {
  ...
  db.prepare(`
    UPDATE materials SET qty_on_hand = ?, qty_on_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?
