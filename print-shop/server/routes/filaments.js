...
router.post('/', (req, res) => {
  ...
  const finalSku = sku || nextSku('filaments', 'filament');
...
router.put('/:id', (req, res) => {
  ...
  if (updates.length) {
    const sql = `UPDATE filaments SET ${updates.map((k) => `${k}=?`).join(', ')}, updated_at=CURRENT_TIMESTAMP WHERE id=?`;
...
router.delete('/:id', (req, res) => {
  ...
  const used = db.prepare(`
    SELECT COUNT(*) AS count FROM item_components
...
router.put('/spools/:spoolId', (req, res) => {
  ...
  const filament = db.prepare('SELECT * FROM filaments WHERE id = ?').get(spool.filament_id);
...
router.delete('/spools/:spoolId', (req, res) => {
  const spool = db.prepare('SELECT * FROM filament_spools WHERE id = ?').get(req.params.spoolId);
  if (!spool) return res.status(404).json({ error: 'Spool not found' });
  db.prepare('DELETE FROM filament_spools WHERE id = ?').run(req.params.spoolId);
  const [updated] = filamentSummary(spool.filament_id);
  res.json({ data: updated });
});
  ...
  const consume = db.transaction(() => {
    ...
    while (remaining > 0) {
      ...
      if (!spool) {
        ...
        db.prepare(`
          UPDATE filament_spools SET status='opened', grams_remaining=?, opened_at=?, updated_at=CURRENT_TIMESTAMP
           WHERE id=?
