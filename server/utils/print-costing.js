...
function getSettings() {
  const rows = db.prepare('SELECT key, value FROM print_settings').all();
...
function computeItemCost(itemId, settings = getSettings(), seen = new Set()) {
  const item = db.prepare('SELECT * FROM print_items WHERE id = ?').get(itemId);
  if (!item) return null;
  const components = db.prepare('SELECT * FROM print_item_components WHERE item_id = ?').all(itemId);
...
function filamentDemandForItem(itemId, seen = new Set()) {
  ...
  const components = db
    .prepare('SELECT * FROM print_item_components WHERE item_id = ?')
    .all(itemId);
