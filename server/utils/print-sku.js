...
function skuPrefix() {
  const row = db.prepare("SELECT value FROM print_settings WHERE key = 'sku_prefix'").get();
...
function nextSpoolCode() {
  const rows = db
    .prepare("SELECT spool_code FROM print_filament_spools WHERE spool_code LIKE 'SPL-%'")
    .all();
  let max = 0;
  ...
  let seq = max + 1;
  const exists = db.prepare('SELECT 1 FROM print_filament_spools WHERE spool_code = ?');
...
function nextOrderNumber() {
  const rows = db.prepare("SELECT order_number FROM print_orders WHERE order_number LIKE 'PO-%'").all();
  let max = 0;
