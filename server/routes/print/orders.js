...
const { nextOrderNumber } = require('../../utils/print-sku');
const { getSettings, priceItem, computeItemCost, round2 } = require('../../utils/print-costing');
const { suggestShipDate, orderProjections, estimatedMinutes } = require('../../utils/print-planning');
...
function orderTotals(orderId) {
  const items = db.prepare(`
    SELECT oi.*, i.name AS item_name, i.sku AS item_sku, i.item_type, i.print_time_minutes
      FROM print_order_items oi LEFT JOIN print_items i ON oi.item_id = i.id
...
function hydrate(order, projectionsById) {
  ...
  const queue = db.prepare(`
    SELECT q.*, i.name AS item_name FROM print_queue q
...
router.get('/', (req, res) => {
  ...
  let sql = 'SELECT * FROM print_orders WHERE 1=1';
  const params = [];
...
router.get('/:id', (req, res) => {
  const order = db.prepare('SELECT * FROM print_orders WHERE id = ?').get(req.params.id);
...
router.post('/', (req, res) => {
  ...
  const create = db.transaction(() => {
    const id = db.prepare(
      `INSERT INTO print_orders (${keys.join(', ')}) VALUES (${keys.map(() => '?').join(', ')})`
    ).run(...keys.map((k) => body[k])).lastInsertRowid;

    const insertLine = db.prepare(`
      INSERT INTO print_order_items (order_id, item_id, description, quantity, unit_price)
      VALUES (?, ?, ?, ?, ?)
    `);
    for (const line of items) {
  ...
  try {
    const id = create();
    const order = db.prepare('SELECT * FROM print_orders WHERE id = ?').get(id);
...
router.put('/:id', (req, res) => {
  ...
  const update = db.transaction(() => {
    ...
    if (keys.length) {
      db.prepare(
        `UPDATE print_orders SET ${keys.map((k) => `${k}=?`).join(', ')}, updated_at=CURRENT_TIMESTAMP WHERE id=?`
      ).run(...keys.map((k) => req.body[k]), req.params.id);
    }

    if (Array.isArray(req.body.items)) {
      db.prepare('DELETE FROM print_order_items WHERE order_id = ?').run(req.params.id);
      const insertLine = db.prepare(`
        INSERT INTO print_order_items (order_id, item_id, description, quantity, unit_price)
        VALUES (?, ?, ?, ?, ?)
      `);
  ...
  const updated = db.prepare('SELECT * FROM print_orders WHERE id = ?').get(req.params.id);
  const { projections } = orderProjections();
...
router.delete('/:id', (req, res) => {
  const order = db.prepare('SELECT * FROM print_orders WHERE id = ?').get(req.params.id);
  if (!order) return res.status(404).json({ error: 'Order not found' });
  db.prepare('DELETE FROM print_orders WHERE id = ?').run(req.params.id);
...
router.post('/:id/queue', (req, res) => {
  ...
  const lines = db.prepare(`
    SELECT oi.*, i.item_type FROM print_order_items oi
      JOIN print_items i ON oi.item_id = i.id
  ...
  const enqueue = db.transaction(() => {
    for (const line of lines) {
      const already = db.prepare(`
        SELECT COUNT(*) AS count FROM print_queue WHERE order_item_id = ? AND status <> 'cancelled'
      `).get(line.id).count;
      ...
      db.prepare(`
        INSERT INTO print_queue (order_id, order_item_id, item_id, quantity, priority, position, estimated_minutes)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(order.id, line.id, line.item_id, line.quantity, req.body.priority || 'normal', position, minutes);
      added += 1;
    }
    if (added > 0 && order.status === 'new') {
      db.prepare("UPDATE print_orders SET status = 'in_production', updated_at = CURRENT_TIMESTAMP WHERE id = ?")
        .run(order.id);
    }
  });
  enqueue();

  const updated = db.prepare('SELECT * FROM print_orders WHERE id = ?').get(order.id);
  const { projections } = orderProjections();
  res.json({
