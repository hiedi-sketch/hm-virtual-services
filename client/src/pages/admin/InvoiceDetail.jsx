    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="card mb-6">
        <div className="flex flex-wrap gap-2">
          {invoice.status === 'draft' && <button onClick={() => updateStatus('sent')} className="btn-primary text-sm">Mark as Sent</button>}
          {['sent', 'overdue'].includes(invoice.status) && <button onClick={() => updateStatus('paid')} className="btn-primary text-sm">Mark as Paid</button>}
          {invoice.status === 'sent' && <button onClick={() => updateStatus('overdue')} className="btn-danger text-sm">Mark Overdue</button>}
          <a href={`/api/invoices/${id}/pdf`} target="_blank" className="btn-secondary text-sm">Download PDF</a>
        </div>
      ...
      <div className="card">
        ...
        <div className="ml-auto max-w-xs space-y-2 text-sm pt-4 border-t border-linen">
          ...
          {invoice.status === 'paid' && invoice.paid_at && (
            <p className="text-green-600 text-xs text-right">Paid on {new Date(invoice.paid_at).toLocaleDateString()}</p>
          )}
        </div>
