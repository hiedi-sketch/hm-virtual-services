...
export default function ClientSidebar({ collapsed, setCollapsed }) {
  const { logout, user, clientData } = useAuth();
  ...
  return (
    ...
    >
      ...
      {!collapsed && clientData && (
        <div className="px-4 py-3 border-b border-white/10">
          <p className="text-white/60 text-xs">Logged in as</p>
          <p className="text-white text-sm font-semibold truncate">{clientData.business_name}</p>
        </div>
      )}
      ...
      <div className="border-t border-white/10 px-3 py-4">
        {!collapsed && <p className="text-white/60 text-xs mb-2 truncate">{user?.email}</p>}
        <button onClick={handleLogout} className="sidebar-link w-full justify-start" title={collapsed ? 'Logout' : undefined}>
