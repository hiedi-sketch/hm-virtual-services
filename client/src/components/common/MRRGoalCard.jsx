...
export default function MRRGoalCard({ current, goal, onGoalUpdate }) {
  ...
  const handleSave = async () => {
    await axios.put('/api/settings/goals', { mrr_goal: newGoal });
  ...
  return (
    <div className="card">
      <div className="flex justify-between items-center mb-2">
        <h2 className="font-bold text-primary">Monthly Revenue Goal</h2>
        <button
      ...
      {editing ? (
        ...
        <p className="text-sm text-black mb-4">Goal: {fmt(goal)}/mo</p>
      )}

      <div className="text-5xl font-bold text-center text-primary mb-4">
        {fmt(current)} <span className="text-black text-2xl">/ {fmt(goal)}</span>
      </div>
